...
PTNode::PTNode(vector<Path> pl, std::map<int, std::set<int> > pr, Instance& instance): instance(instance){
	plan = pl;
	priority = pr;
}
...
void PTNode::calculateCost(){
	...
	double c = 0;
	int index = 0;
	for(auto it = plan.begin(); it != plan.end(); ++it, ++index){
		c += it->back().leaving_time_tail*instance.agents[index].importance;
	}
	cost = c;
}
