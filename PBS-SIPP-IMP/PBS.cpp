...
void PBS::initializePriority(std::map<int, std::set<int> >& p,std::map<int, std::map<int, std::vector<int> > >& ta){
	std::vector<int> higherPriorityVehicles;
	std::vector<int> lowerPriorityVehicles;
	for(auto it = arrivingVehicles.begin(); it != arrivingVehicles.end(); ++it){
		if(it->importance == 2) higherPriorityVehicles.push_back(it->id);
		else lowerPriorityVehicles.push_back(it->id);
	}
	for(auto it = trajectoryToAgent.begin(); it != trajectoryToAgent.end(); ++it){
		for(auto it2 = it->second.begin(); it2 != it->second.end(); ++it2){
			for(auto it3 = it2->second.begin(); it3 != it2->second.end(); ++it3){
	...
	}	for(int i = 0; i < higherPriorityVehicles.size(); ++i){
		for(int j = 0; j < lowerPriorityVehicles.size(); ++j){
			if(p[higherPriorityVehicles[i]].find(lowerPriorityVehicles[j]) == p[higherPriorityVehicles[i]].end() 
				and p[lowerPriorityVehicles[j]].find(higherPriorityVehicles[i]) == p[lowerPriorityVehicles[j]].end()){
				p[higherPriorityVehicles[i]].insert(lowerPriorityVehicles[j]);
			}
		}
	}
