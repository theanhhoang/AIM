...
{
return a.earliest_start_time < b.earliest_start_time;
}

void PBS::initializePriority(std::map<int, std::set<int> >& p,std::map<int, std::map<int, std::vector<int> > >& ta){
    std::sort(arrivingVehicles.begin(), arrivingVehicles.end(), compareAgents);
    for(auto it = arrivingVehicles.begin(); it != arrivingVehicles.end(); ++it){
        for(auto it2 = std::next(it); it2 != arrivingVehicles.end(); ++it2){
            p[it->id].insert(it2->id);
        }
    }
}
